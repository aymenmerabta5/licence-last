...
import { loginAsCompany, loginAsSuperAdmin } from "../fixtures/auth"
...
test.describe("Company Approval/Rejection/Suspension", () => {
  ...
  test("can suspend and reactivate an approved company", async ({ page }) => {
    await expect(page).toHaveURL(/\/en\/dashboard\/admin\/companies/, {
      timeout: 15000,
    })
    ...
    if (await suspendButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const reactivateButton = page
        ...
        .first()
      if (
        await reactivateButton.isVisible({ timeout: 5000 }).catch(() => false)
      ) {
  ...
  test("company admin can access dashboard when approved", async ({ page }) => {
