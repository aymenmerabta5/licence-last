...
export const dialogSizeVariants = cva("max-w-[calc(100%-2rem)] rounded-xl p-4 ring-1", {
  variants: {
    size: {
      sm: "sm:max-w-xs",
      md: "sm:max-w-sm",
...
export const dropdownContentClassName =
  "data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[side=inline-start]:slide-in-from-right-2 data-[side=inline-end]:slide-in-from-left-2 bg-background/95 text-popover-foreground min-w-32 rounded-xl border border-border/40 p-1.5 shadow-xl backdrop-blur-xl ring-1 ring-foreground/10 duration-100 z-50 max-h-(--available-height) w-(--anchor-width) origin-(--transform-origin) overflow-x-hidden overflow-y-auto outline-none data-closed:overflow-hidden"
