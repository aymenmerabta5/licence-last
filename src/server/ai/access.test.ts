...
describe("access", () => {
  describe("isRoleAllowedForIntent", () => {
    ...
    test("denies when role is null or undefined", () => {
      expect(
        isRoleAllowedForIntent({ role: null, intent: "offer_generate_draft" }),
      ).toBe(false)
      expect(
        isRoleAllowedForIntent({
          role: undefined,
    ...
    test("defaults to company_admin for null intent", () => {
      expect(
        isRoleAllowedForIntent({ role: "company_admin", intent: null }),
      ).toBe(true)
      expect(isRoleAllowedForIntent({ role: "student", intent: null })).toBe(
        false,
      )
    })
