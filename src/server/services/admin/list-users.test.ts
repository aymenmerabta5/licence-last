...
describe("listUsers", () => {
  ...
  test("should use default limit and offset", async () => {
    ...
    await listUsers({}, { authApi: { listUsers: mockListUsers }, getHeaders: mockHeaders })
  ...
  test("should pass custom limit and offset", async () => {
    ...
    await listUsers({ limit: 50, offset: 10 }, { authApi: { listUsers: mockListUsers }, getHeaders: mockHeaders })
  ...
  test("should include search params when searchValue is provided", async () => {
    ...
    await listUsers({ searchValue: "test@", searchField: "email" }, { authApi: { listUsers: mockListUsers }, getHeaders: mockHeaders })

    const call = (mockListUsers.mock.calls as unknown[][])[0][0] as {
      query: { searchValue?: string; searchField?: string; searchOperator?: string }
    }
  ...
  test("should include sort params when sortBy is provided", async () => {
    ...
    await listUsers({ sortBy: "name", sortDirection: "desc" }, { authApi: { listUsers: mockListUsers }, getHeaders: mockHeaders })
  ...
  test("should include filter params when filterField is provided", async () => {
    ...
    await listUsers({ filterField: "role", filterValue: "student", filterOperator: "eq" }, { authApi: { listUsers: mockListUsers }, getHeaders: mockHeaders })
