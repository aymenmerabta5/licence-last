...
describe("removeUser", () => {
  ...
  test("should call auth.api.removeUser with userId", async () => {
    ...
    await removeUser("user-1", { authApi: { removeUser: mockRemoveUser }, getHeaders: mockHeaders })
  ...
  test("should return result from auth API", async () => {
    ...
    const result = await removeUser("user-1", { authApi: { removeUser: mockRemoveUser }, getHeaders: mockHeaders })
